...
const stressAvg = JSON.stringify(history.map(h => parseFloat(h.stress?.avg_duration) || 0));
const stressP95 = JSON.stringify(history.map(h => parseFloat(h.stress?.p95_duration) || 0));
const stressMax = JSON.stringify(history.map(h => parseFloat(h.stress?.max_duration) || 0));
const spikeAvg = JSON.stringify(history.map(h => parseFloat(h.spike?.avg_duration) || 0));
const spikeP95 = JSON.stringify(history.map(h => parseFloat(h.spike?.p95_duration) || 0));
const spikeMax = JSON.stringify(history.map(h => parseFloat(h.spike?.max_duration) || 0));
const stressErrors = JSON.stringify(history.map(h => parseFloat(h.stress?.error_rate) || 0));
const spikeErrors = JSON.stringify(history.map(h => parseFloat(h.spike?.error_rate) || 0));
const stressRPS = JSON.stringify(history.map(h => parseFloat(h.stress?.rps) || 0));
const spikeRPS = JSON.stringify(history.map(h => parseFloat(h.spike?.rps) || 0));
const stressReqs = JSON.stringify(history.map(h => parseInt(h.stress?.requests) || 0));
const spikeReqs = JSON.stringify(history.map(h => parseInt(h.spike?.requests) || 0));
...
if (history.length >= 2) {
    ...
    if (curr.stress && prev.stress) {
        const avgPct = prev.stress.avg_duration > 0 ? ((curr.stress.avg_duration - prev.stress.avg_duration) / prev.stress.avg_duration * 100).toFixed(1) : '0';
        const p95Pct = prev.stress.p95_duration > 0 ? ((curr.stress.p95_duration - prev.stress.p95_duration) / prev.stress.p95_duration * 100).toFixed(1) : '0';
        compSection = '<div class="comparison-section"><h3>Latest Run vs Previous Run</h3><p class="comp-dates">Comparing:  <strong>' + curr.date + '</strong> vs <strong>' + prev.date + '</strong></p><div class="comp-grid"><div class="comp-item"><div class="comp-value">' + curr.stress.avg_duration + 'ms</div><div class="comp-label">Avg Response</div><div class="comp-prev">was ' + prev.stress.avg_duration + 'ms</div><div class="comp-change ' + (parseFloat(avgPct) > 0 ? 'worse' : 'better') + '">' + (parseFloat(avgPct) > 0 ? '↑' : '↓') + ' ' + Math.abs(avgPct) + '%</div></div><div class="comp-item"><div class="comp-value">' + curr.stress.p95_duration + 'ms</div><div class="comp-label">P95 Response</div><div class="comp-prev">was ' + prev.stress.p95_duration + 'ms</div><div class="comp-change ' + (parseFloat(p95Pct) > 0 ? 'worse' : 'better') + '">' + (parseFloat(p95Pct) > 0 ? '↑' : '↓') + ' ' + Math.abs(p95Pct) + '%</div></div><div class="comp-item"><div class="comp-value">' + curr.stress.error_rate + '%</div><div class="comp-label">Error Rate</div><div class="comp-prev">was ' + prev.stress.error_rate + '%</div></div><div class="comp-item"><div class="comp-value">' + curr.stress.rps + '</div><div class="comp-label">Requests/sec</div><div class="comp-prev">was ' + prev.stress.rps + '</div></div></div></div>';
    }
}
...
history.slice().reverse().forEach(function(run) {
    if (run.stress) {
        const ec = parseFloat(run.stress.error_rate) < 1 ? 'good' : parseFloat(run.stress.error_rate) < 5 ? 'warn' : 'bad';
        tableRows += '<tr><td>' + run.date + '</td><td><span class="badge stress">Stress</span></td><td>' + Number(run.stress.requests).toLocaleString() + '</td><td class="' + ec + '">' + run.stress.error_rate + '%</td><td>' + run.stress.avg_duration + 'ms</td><td>' + run.stress.med_duration + 'ms</td><td>' + run.stress.p95_duration + 'ms</td><td>' + run.stress.max_duration + 'ms</td><td>' + run.stress.rps + '</td><td>' + run.stress.data_received + 'MB</td></tr>';
    }
    if (run.spike) {
        const ec = parseFloat(run.spike.error_rate) < 1 ? 'good' : parseFloat(run.spike.error_rate) < 5 ? 'warn' : 'bad';
        tableRows += '<tr><td>' + run.date + '</td><td><span class="badge spike">Spike</span></td><td>' + Number(run.spike.requests).toLocaleString() + '</td><td class="' + ec + '">' + run.spike.error_rate + '%</td><td>' + run.spike.avg_duration + 'ms</td><td>' + run.spike.med_duration + 'ms</td><td>' + run.spike.p95_duration + 'ms</td><td>' + run.spike.max_duration + 'ms</td><td>' + run.spike.rps + '</td><td>' + run.spike.data_received + 'MB</td></tr>';
    }
});
